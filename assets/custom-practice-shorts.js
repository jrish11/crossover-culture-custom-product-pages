(() => {
  ...
  const initializeSection = (section) => {
    ...
    const validateForm = () => {
      ...
      if (!isValid) {
        setMessage('error', pricingIssue || `Please confirm at least ${minPieces} total pieces, choose valid colors, and upload one vector artwork file before adding to cart.`);
      }
